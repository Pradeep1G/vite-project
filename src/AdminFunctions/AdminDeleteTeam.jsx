...
function AdminDeleteTeam() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!teamId) {
      setError('Team ID must be in the format CSE-yy-NNNN, where "yy" is the current year and "NNNN" is less than 1700.');
      return;
    }
    ...
    try {
        setIsLoading(true);
      const response = await axios.post(serverPath1+'/deleteTeam', {
        teamId:teamId
      });
      console.log(teamId);
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      const func=async()=>{
        setIsLoading(true);
        const response = await axios.get(serverPath1+"/checkAuthentication/"+adminMailId, {headers});
        setIsLoading(false)
  ...
  <div className="lg:w-1/4 md:w-2/4 s:w-2/4 xs:w-3/4 border p-4 bg-white bg-opacity-50 backdrop-filter rounded-lg shadow-lg">
    <div className="block">
      <div className="flex justify-center">
        <h1 className="p-4 font-semibold text-2xl text-center">Enter Team ID</h1>
      </div>

...
function AdminAddTeam() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
        setIsLoading(true);
      const response = await axios.post(serverPath1 + '/staffLogin/staffDashboard/selectStudent/'+facultyEmail, formData);
        ...
        <div className="lg:w-1/4 md:w-2/4 s:w-2/4 xs:w-3/4 border p-4 bg-white bg-opacity-50 backdrop-filter rounded-lg shadow-lg">
          <div className="block">
            <div className="flex justify-center">
              <h1 className="p-4 font-semibold text-2xl text-center">Add a Team</h1>
            </div>
            <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
              ...
              <div className="flex items-center space-x-2">
                <label htmlFor="teamSize" className="text-sm">Team Size (2):</label>
                <input
                  type="checkbox"
                  id="teamSize"
                  name="teamSize"
